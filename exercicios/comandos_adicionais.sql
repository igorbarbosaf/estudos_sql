...
select
	nome,
	coalesce(cast(extract(month from data_nascimento) as varchar), 'Não informado') as mes_nascimento
from
	cliente;
...
select
	nome,
	coalesce(cast(extract(year from data_nascimento) as varchar), 'Não informado') as data_nascimento
from
	cliente;
